...
public class Finder {
                ...
                if (i < input.length()) {// если не достигли конца строки, то ...  ; иначе ничего не делаем
                    ...
                    if (isServiceCharacter(currentChar)) {//например: "car   !black" --> "car!black"
                        if ((currentChar == '(') || (currentChar == '!'))
                            filteredStr += "&" + currentChar;//например при    "!(the|memory) (leaks | do)"  --> !(the|memory)&(leaks|do)
                        else filteredStr += currentChar;
                    }
                }
            } else filteredStr += currentChar;
    ...
    private void loadIndexBaseFromFile() {
        try {
            FileInputStream fis = new FileInputStream(defaultSaveIndexDirectory + File.separator + savedIndexFileName);//если файла не существует - будет брошен FileNotFoundException
    ...
    private void saveIndexBaseToFile() {
        try {
            ...
            File file = new File(defaultSaveIndexDirectory + File.separator + savedIndexFileName);
            if (file.exists()) {
                file.delete(); //всегда заново пишем объект полностью, а не добавляем что-то
            }
            file.createNewFile();
            FileOutputStream fos = new FileOutputStream(defaultSaveIndexDirectory + File.separator + savedIndexFileName);
